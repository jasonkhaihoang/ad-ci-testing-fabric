...
The results path comes from DBT_QUALITY_DUCKDB_PATH — the same env var the
dbt-quality profile consumes via env_var() — so the writer and this reader
cannot drift to different filenames (AC-85). A missing/unreadable results
database is reported as a problem, never rendered as an all-clear (AC-83/84).
"""
