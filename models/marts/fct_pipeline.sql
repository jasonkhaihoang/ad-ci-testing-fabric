...
opportunity_metrics as (
    select
        ...
        case
            ...
            then datediff(day, cast(opp.created_date as date), opp.close_date)
            else null
        end as sales_cycle_days,

        -- Calculated: Age of opportunity
        datediff(day, cast(opp.created_date as date), current_date()) as opportunity_age_days,
        ...
        case
            ...
            then datediff(day, cast(opp.last_stage_change_date as date), current_date())
            else null
